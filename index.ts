...
const subgraphNameToConfigs: Record<string, Record<string, SubgraphConfig[]>> = {};
configFiles.forEach(configFile => {
  const subgraphConfig = getSubgraphConfig(configFile);
  const subgraphName = subgraphConfig.subgraphName;
  const deploymentId = subgraphConfig.deploymentId;

  // Add the subgraph name if it doesn't exist
  if (!subgraphNameToConfigs[subgraphName]) {
    subgraphNameToConfigs[subgraphName] = {};
  }

  // Add the deployment id if it doesn't exist
  if (!subgraphNameToConfigs[subgraphName][deploymentId]) {
    subgraphNameToConfigs[subgraphName][deploymentId] = [];
  }

  // Add the subgraph config
  subgraphNameToConfigs[subgraphName][deploymentId].push(subgraphConfig);
});

// Iterate over each subgraph
for (const subgraphName in subgraphNameToConfigs) {
  const deploymentIdToConfigs = subgraphNameToConfigs[subgraphName];
  ...
  for (const deploymentId in deploymentIdToConfigs) {
    const subgraphConfigs = deploymentIdToConfigs[deploymentId];
    ...
    for (const subgraphConfig of subgraphConfigs) {
